...
class EarlyStopping:
    ...
    cloned to CPU memory (``self.best_model``) and persisted to disk at
    ...
    Attributes:
        ...
        best_model: In-memory CPU copy of the best ``state_dict``.
    ...
    def _snapshot_state_dict_to_cpu(model: nn.Module) -> Dict[str, torch.Tensor]:
        """Return a CPU-side clone of ``model.state_dict()``.

        Avoids ``copy.deepcopy`` directly on a GPU ``state_dict`` which would
        require ``2x`` of the resident GPU memory transiently and tends to
        OOM on multi-tenant training nodes (each tensor's ``__deepcopy__``
        eventually calls ``self.clone()`` on the same device). Streaming each
        parameter through ``.detach().cpu()`` keeps the peak GPU footprint
        bounded by a single parameter at a time.
        """
        cpu_state: Dict[str, torch.Tensor] = {}
        for k, v in model.state_dict().items():
            if isinstance(v, torch.Tensor):
                cpu_state[k] = v.detach().to('cpu', copy=True)
            else:
    ...
    def __call__(
        ...
        if self.best_score is None:
            ...
            # Snapshot the parameters on CPU to keep GPU memory bounded
            # (the disk checkpoint is the source of truth; this is just an
            # in-memory cache for faster best-model restore at the end).
            self.best_model = self._snapshot_state_dict_to_cpu(model)
