...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # Linear warmup (always) optionally followed by cosine decay for the
        # AdamW (dense) optimizer. Cosine decay is the recommended default
        # post-warmup behaviour: holding LR constant after warmup left the
        # later epochs free to drift the parameters into a worse basin once
        # train loss had already plateaued (observed in tensorboard runs).
        self._lr_schedule = lr_schedule
        self._warmup_steps = max(0, warmup_steps)
        self._cosine_min_lr_ratio = cosine_min_lr_ratio
        # We need an estimate of total steps to anchor the cosine decay; if
        # the loader has no length we fall back to a long-but-finite horizon
        # so the schedule still decays meaningfully.
        try:
            self._steps_per_epoch = max(1, len(train_loader))
        except TypeError:
            self._steps_per_epoch = 10000
        self._total_steps = self._steps_per_epoch * max(1, num_epochs)
        ...
        def _lr_lambda(step: int) -> float:
            ...
            if self._lr_schedule == 'cosine':
                progress_denom = max(1, self._total_steps - warmup)
                progress = float(step - warmup) / float(progress_denom)
                progress = min(1.0, max(0.0, progress))
                cos_factor = 0.5 * (1.0 + math.cos(math.pi * progress))
                min_ratio = self._cosine_min_lr_ratio
                return min_ratio + (1.0 - min_ratio) * cos_factor
            return 1.0

        if self._warmup_steps > 0 or lr_schedule == 'cosine':
            self._dense_scheduler = torch.optim.lr_scheduler.LambdaLR(
                self.dense_optimizer,
                lr_lambda=_lr_lambda,
            )
            logging.info(
                f"LR schedule: warmup={self._warmup_steps}, "
                f"mode={lr_schedule}, total_steps={self._total_steps}, "
                f"min_ratio={self._cosine_min_lr_ratio}"
            )
        else:
            self._dense_scheduler = None

        # Stochastic Weight Averaging (SWA) — averages model parameters across
        # the tail epochs of training, which typically tightens the val→test
        # generalization gap by selecting a flatter minimum. Implementation
        # uses ``torch.optim.swa_utils.AveragedModel`` (equal-weight running
        # mean of parameter tensors). The averaged model is what we evaluate
        # and persist after epoch >= swa_start_epoch.
        self.use_swa = use_swa
        self.swa_start_epoch = swa_start_epoch
        self.swa_updates = 0
        if use_swa:
            from torch.optim.swa_utils import AveragedModel
            self.swa_model: Optional[nn.Module] = AveragedModel(model)
            logging.info(
                f"SWA enabled: averaging starts at end of epoch "
                f"{swa_start_epoch} (using equal-weight running mean)."
            )
        else:
            self.swa_model = None
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"use_swa={use_swa}, swa_start_epoch={swa_start_epoch}, "
    ...
    def _handle_validation_result(
        ...
        """
        # The model whose state_dict gets saved. Defaults to the live model
        # being trained, but may be overridden with the SWA-averaged model
        # so that what we evaluate is exactly what we serialize for inference.
        save_model = eval_model if eval_model is not None else self.model
        ...
        if not is_likely_new_best:
            ...
            self.early_stopping(val_auc, save_model, {
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                # Mid-epoch evals always score the live model, never SWA;
                # SWA's running mean only updates at clean epoch boundaries.
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    logging.info(f"Step {total_step} Validation | AUC: {val_auc}, LogLoss: {val_logloss} [raw]")
                    ...
                    if self.early_stopping.early_stop:
                        ...
                        # Promote the SWA-averaged weights into the live
                        # model on early exit so the trailing logging /
                        # checkpoint routines see SWA values.
                        self._maybe_finalize_swa()
                        return

            logging.info(f"Epoch {epoch}, Average Loss: {loss_sum / len(self.train_loader)}")

            # End-of-epoch SWA update + evaluation. Once SWA has started,
            # we evaluate (and persist) the SWA-averaged model rather than
            # the live model — that's the model we want at inference time.
            if self.use_swa and epoch >= self.swa_start_epoch:
                self.swa_model.update_parameters(self.model)
                self.swa_updates += 1
                logging.info(
                    f"SWA: averaged epoch {epoch} into running mean "
                    f"(total updates so far: {self.swa_updates})"
                )
                eval_model = self.swa_model.module
                eval_label = "SWA"
            else:
                eval_model = self.model
                eval_label = "raw"

            val_auc, val_logloss = self.evaluate(epoch=epoch, eval_model=eval_model)
            self.model.train()
            torch.cuda.empty_cache()

            logging.info(f"Epoch {epoch} Validation | AUC: {val_auc}, LogLoss: {val_logloss} [{eval_label}]")
            ...
            self._handle_validation_result(
                total_step, val_auc, val_logloss, eval_model=eval_model)
            ...
            # Restore best weights even between epochs would be wrong (we'd
            # lose the in-progress optimizer state context). Restoration is
            # only done once, at the very end of train(). See bottom of method.
    ...
    def _maybe_finalize_swa(self) -> None:
        """No-op placeholder kept for symmetry / future BN-stats updates.

        We use LayerNorm everywhere (no running statistics to refresh), so
        this currently does nothing. It exists so that anyone adding BN
        layers later has an obvious hook to call ``update_bn`` on the
        validation loader, and so the early-exit paths in ``train()`` can
        emit a single, consistent finalization point.
        """
        if self.swa_model is None:
            return
        logging.info(
            f"SWA finalize: {self.swa_updates} epoch(s) averaged into the "
            f"persisted checkpoint."
        )
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self._dense_scheduler is not None:
            self._dense_scheduler.step()
    ...
    def evaluate(
        ...
        Args:
            epoch: optional epoch index for logging only.
            eval_model: model to evaluate. Defaults to ``self.model``; pass
                ``self.swa_model.module`` once SWA averaging has begun so the
                reported metric reflects the SWA-averaged weights that will
                be persisted by ``_handle_validation_result``.
        """
        model = eval_model if eval_model is not None else self.model
        ...
        with torch.no_grad():
            for step, batch in pbar:
                logits, labels = self._evaluate_step(batch, eval_model=model)
    ...
    def _evaluate_step(
        ...
        model_input = self._make_model_input(device_batch)
        logits, _ = model.predict(model_input)  # (B, 1), (B, D)
        logits = logits.squeeze(-1)  # (B,)
