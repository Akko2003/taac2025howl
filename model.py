...
class CrossNetV2(nn.Module):
    """DCN-V2 Cross Network for explicit high-order feature interactions.
    ...
    where ``W_l`` is a square matrix (``input_dim``, ``input_dim``). After
    ``L`` layers the output captures up to ``(L+1)``-order multiplicative
    interactions between the raw input features. This is *complementary* to
    the deep / attention towers which model interactions implicitly: the
    Cross network gives an explicit polynomial-style basis that the deep
    tower would otherwise have to discover from scratch through
    composition.

    The optional ``low_rank`` argument enables the DCN-V2 mixture-of-experts
    low-rank factorization (W_l ~= V_l U_l with ``U_l`` of shape
    (input_dim, low_rank) and ``V_l`` of shape (low_rank, input_dim)),
    which trades a small amount of representation capacity for a large
    parameter savings — useful when ``input_dim`` is in the thousands.
    """
    ...
    def __init__(
        ...
        self.use_low_rank = low_rank is not None and 0 < low_rank < input_dim
        ...
        if self.use_low_rank:
            ...
            self.V = nn.ModuleList([
                nn.Linear(low_rank, input_dim)
                for _ in range(num_layers)
            ])
            # Bias term lives inside V[l] (the second factor), so b_l is
            # absorbed there and no separate bias parameter is needed.
        else:
        ...
        # LayerNorm between cross layers stabilizes the residual sum when
        # the multiplicative term grows quickly across layers (the original
        # paper does not use this, but in practice it prevents the early
        # cross outputs from dominating the deep-tower logit during the
    ...
    def forward(self, x0: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x0: (B, input_dim) flattened raw feature vector.

        Returns:
            (B, input_dim) tensor encoding up to (num_layers+1)-order
            multiplicative interactions of x0.
        """
        x = x0
        for layer_idx in range(self.num_layers):
            if self.use_low_rank:
                projected = self.V[layer_idx](self.U[layer_idx](x))
            else:
                projected = self.W[layer_idx](x)
            x = x0 * projected + x
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Training stability (from DualStream experiments)
        class_prior_logit: float = -2.0,
        use_seq_agg: bool = True,
        # DCN-V2 explicit-cross network (parallel branch on flattened NS+dense)
        use_dcn_v2: bool = False,
        dcn_v2_layers: int = 3,
        dcn_v2_low_rank: int = 0,
    ) -> None:
        super().__init__()
        ...
        self.num_queries = num_queries
        self.class_prior_logit = class_prior_logit
        self.use_seq_agg = use_seq_agg
        ...
        # Seq aggregation shortcut (DualStream bypass): per-domain pooling
        # over sequence tokens, projected to d_model, added as a residual to
        # the transformer output. The pooling is now target-aware
        # (DIN-style): the item-side context vector is used as the attention
        # query so the bypass focuses on items most relevant to the candidate
        # rather than averaging uniformly. This was the biggest single
        # improvement from local experiments.
        if use_seq_agg:
            # Per-domain DIN-style attention scorer
            self.seq_agg_attn = nn.ModuleList([
                nn.Sequential(
            ...
            self.seq_agg_proj = nn.Sequential(
                nn.Linear(self.num_sequences * d_model, d_model, bias=False),
                nn.LayerNorm(d_model),
                nn.SiLU(),
            )

        # SENet feature gate over NS tokens: dynamically reweights each NS
        # token's channels based on a global summary of all NS tokens. Adds
        # negligible parameters but consistently helps in CVR/CTR tasks.
        # Squeeze: average each token to scalar, then project to (num_ns,)
        # gates which are then expanded back to per-token-channel scaling.
        se_hidden = max(self.num_ns // 2, 4)
        self.ns_se_gate = nn.Sequential(
            nn.Linear(self.num_ns, se_hidden),
            nn.SiLU(),
            nn.Linear(se_hidden, self.num_ns),
            nn.Sigmoid(),
        )

        # ================== DCN-V2 explicit cross network (parallel) ====
        # The deep + attention towers above implicitly learn feature
        # interactions through composition; the cross network learns them
        # explicitly via repeated multiplicative residuals on the raw
        # flattened feature vector (gated NS tokens + raw dense features).
        # Outputs a separate logit which is *added* to the deep logit at
        # the end of forward() — a "two-tower fusion" arrangement that
        # tends to lift held-out AUC by ~0.005-0.02 on CTR/CVR tasks.
        self.use_dcn_v2 = use_dcn_v2
        if use_dcn_v2:
            cross_input_dim = self.num_ns * d_model
            if self.has_user_dense:
                cross_input_dim += user_dense_dim
            if self.has_item_dense:
                cross_input_dim += item_dense_dim
            self._cross_input_dim = cross_input_dim
            low_rank = dcn_v2_low_rank if dcn_v2_low_rank > 0 else None
            self.cross_net = CrossNetV2(
                input_dim=cross_input_dim,
                num_layers=dcn_v2_layers,
                low_rank=low_rank,
            )
            # Project the cross output to a logit. Mirrors the deep tower's
            # classifier shape so the two logits are directly summable.
            self.cross_logit_head = nn.Sequential(
                nn.Linear(cross_input_dim, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
                nn.Dropout(dropout_rate),
                nn.Linear(d_model, action_num),
            )
            # Zero-init the final layer of the cross head so the model
            # starts equivalent to the existing deep-only tower; the cross
            # branch only contributes once it has learned something useful.
            nn.init.zeros_(self.cross_logit_head[-1].weight)
            nn.init.zeros_(self.cross_logit_head[-1].bias)
            logging.info(
                f"DCN-V2 enabled: input_dim={cross_input_dim}, "
                f"layers={dcn_v2_layers}, low_rank={low_rank}"
            )
        else:
            self.cross_net = None
            self.cross_logit_head = None
    ...
    def _init_params(self) -> None:
        ...
        # Initialize output bias with class prior to prevent all-negative collapse on
        # imbalanced CVR data (typical positive rate ~10-15%, sigmoid(-2.0) ≈ 12%).
        if self.class_prior_logit != 0.0:
            nn.init.constant_(self.clsfier[-1].bias, self.class_prior_logit)
            logging.info(f"Output bias initialized with class_prior_logit={self.class_prior_logit:.3f} "
                         f"(≈{torch.sigmoid(torch.tensor(self.class_prior_logit)).item():.1%} prior positive rate)")
    ...
    def _target_aware_pool(
        self,
        seq_tokens_list: List[torch.Tensor],
        seq_masks_list: List[torch.Tensor],
        target_repr: torch.Tensor,
    ) -> torch.Tensor:
        """DIN-style attention pooling per sequence domain.

        For each domain, scores each behavior token by an MLP fed
        [token, target, token*target] (Hadamard interaction is a strong
        signal of relevance), masks padding to -inf, softmaxes, and reduces
        the sequence to a single (B, D) vector.

        Args:
            seq_tokens_list: per-domain (B, L_i, D) tensors.
            seq_masks_list: per-domain (B, L_i) bool tensors, True = padding.
            target_repr: (B, D), candidate-item representation used as
                attention query.

        Returns:
            (B, S*D) concatenation of pooled per-domain vectors.
        """
        pooled_list: List[torch.Tensor] = []
        B = target_repr.shape[0]
        for i, (tokens, mask) in enumerate(zip(seq_tokens_list, seq_masks_list)):
            L = tokens.shape[1]
            tgt = target_repr.unsqueeze(1).expand(-1, L, -1)  # (B, L, D)
            attn_in = torch.cat([tokens, tgt, tokens * tgt], dim=-1)  # (B, L, 3D)
            scores = self.seq_agg_attn[i](attn_in).squeeze(-1)  # (B, L)
            # Mask padding before softmax. Avoid -inf rows that would
            # produce NaN by ensuring at least one position is finite per row.
            all_pad = mask.all(dim=1, keepdim=True)  # (B, 1)
            safe_scores = scores.masked_fill(mask, float('-inf'))
            # Where the entire row is padding, fall back to a uniform score
            # so softmax is well defined; the pooled value will be ~0 because
            # the tokens at those positions are themselves zero / arbitrary.
            safe_scores = torch.where(
                all_pad.expand_as(safe_scores),
                torch.zeros_like(safe_scores),
                safe_scores,
            )
            attn_w = F.softmax(safe_scores, dim=-1)  # (B, L)
            pooled = (tokens * attn_w.unsqueeze(-1)).sum(dim=1)  # (B, D)
            pooled_list.append(pooled)
        return torch.cat(pooled_list, dim=-1)  # (B, S*D)

    def _apply_ns_se_gate(self, ns_tokens: torch.Tensor) -> torch.Tensor:
        """Apply SENet-style per-token gating to NS tokens.

        Squeezes each NS token to a scalar (mean over channels), passes
        through a small MLP, and uses the per-token gate to reweight the
        original NS tokens. Helps the network suppress noisy / redundant
        feature groups.
        """
        # ns_tokens: (B, num_ns, D)
        squeezed = ns_tokens.mean(dim=-1)  # (B, num_ns)
        gates = self.ns_se_gate(squeezed)  # (B, num_ns)
        # Centre around 1.0 so we start near identity (sigmoid is in [0,1]).
        gates = gates * 2.0
        return ns_tokens * gates.unsqueeze(-1)

    def _compute_cross_logit(
        self,
        ns_tokens: torch.Tensor,
        inputs: ModelInput,
    ) -> torch.Tensor:
        """Run the DCN-V2 parallel branch and return its logit contribution.

        Args:
            ns_tokens: (B, num_ns, D) gated NS-token block. Already passed
                through the SENet gate so that the cross network operates
                in the same "filtered" feature space the deep tower sees.
            inputs: original ``ModelInput`` (used to grab the raw dense
                feature vectors that bypass the dense token projection).

        Returns:
            (B, action_num) cross-network logit. Zero-init on the head's
            final layer means this contribution is identically zero at
            step 0 and only grows as the cross branch learns useful
            interactions.
        """
        B = ns_tokens.shape[0]
        flat_parts = [ns_tokens.reshape(B, -1)]  # (B, num_ns * D)
        if self.has_user_dense:
            flat_parts.append(inputs.user_dense_feats)
        if self.has_item_dense:
            flat_parts.append(inputs.item_dense_feats)
        x0 = torch.cat(flat_parts, dim=-1)  # (B, cross_input_dim)
        cross_out = self.cross_net(x0)
        return self.cross_logit_head(cross_out)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # SENet-style gate over NS tokens (learnable per-token reweighting).
        ns_tokens = self._apply_ns_se_gate(ns_tokens)
        ...
        # 3b. Seq aggregation shortcut: target-aware (DIN-style) attention pool
        #     per domain, with the candidate-item representation as the query.
        if self.use_seq_agg:
            target_repr = item_ns.mean(dim=1)  # (B, D)
            seq_agg_in = self._target_aware_pool(
                seq_tokens_list, seq_masks_list, target_repr)
        ...
        # 4b. Add seq aggregation shortcut as residual (DualStream bypass connection).
        if self.use_seq_agg:
            output = output + self.seq_agg_proj(seq_agg_in)

        # 5. Classifier
        logits = self.clsfier(output)  # (B, action_num)

        # 6. DCN-V2 parallel branch: explicit high-order feature crosses.
        if self.use_dcn_v2:
            logits = logits + self._compute_cross_logit(ns_tokens, inputs)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # DCN-V2 parallel branch must mirror what forward() does or
        # train/eval logits will diverge.
        if self.use_dcn_v2:
            logits = logits + self._compute_cross_logit(ns_tokens, inputs)
